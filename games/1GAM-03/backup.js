...
function game()
{
		...
		function init() 
		{
			...
			[			
				...
				{src:"assets/sound/mainTheme.ogg|assets/sound/mainTheme.ogg",id:"mainTheme"},
				{src:"assets/sound/bossTheme.ogg|assets/sound/bossTheme.ogg",id:"bossTheme"},

				{src:"assets/sound/button.ogg|assets/sound/button.ogg",id:"button",data:20},
				{src:"assets/sound/candy.ogg|assets/sound/candy.ogg",id:"candy",data:20},
				{src:"assets/sound/playerShoot.ogg|assets/sound/playerShoot.ogg",id:"playerShoot",data:20},
				{src:"assets/sound/enemyShoot.ogg|assets/sound/enemyShoot.ogg",id:"enemyShoot",data:20},
				{src:"assets/sound/enemyDamage.ogg|assets/sound/enemyDamage.ogg",id:"enemyDamage",data:20},
				{src:"assets/sound/enemyDeath.ogg|assets/sound/enemyDeath.ogg",id:"enemyDeath",data:20},
				{src:"assets/sound/levelUp.ogg|assets/sound/levelUp.ogg",id:"levelUp",data:20},
				{src:"assets/sound/life.ogg|assets/sound/life.ogg",id:"life",data:20},
				{src:"assets/sound/playerDamage.ogg|assets/sound/playerDamage.ogg",id:"playerDamage",data:20},
				{src:"assets/sound/playerDeath.ogg|assets/sound/playerDeath.ogg",id:"playerDeath",data:20},
				{src:"assets/sound/rugbyTouch.ogg|assets/sound/rugbyTouch.ogg",id:"rugbyTouch",data:20}				
			];
		...
		function menu()
		{
			curtains.open(function() {
				...
				var mess= new createjs.Text("GOBLIN RAMPAGE","bold 80px Geo","yellow");
				mess.x=150;
				mess.y=100;
				...
				c.addChild(mess,playBt);
				stage.addChildAt(c,0);
				if(!music){
				music=createjs.Sound.play("mainTheme",createjs.Sound.INTERRUPT_ANY, 0, 0, -1, 1, 0);
				music.setVolume(0.5);
				}
					/*var selectBt=new createjs.BitmapAnimation(elements.buttons);
					selectBt.gotoAndStop("select");
				...
				to({x:900},200,createjs.Ease.QuadIn).call(function(){
					...
					restartBt.onClick=function(){
						removeThings(upperContainer,function(){
							upperContainer.removeChild(menuBt);
					...
					nextBt.onClick=function(){
						removeThings(upperContainer,function(){upperContainer.removeChild(menuBt);gameIstance(++gameLevel);});
					}
					var menuBt=new createjs.BitmapAnimation(elements.buttons);
					menuBt.gotoAndStop("menu");
					menuBt.onClick=function(){
						removeThings(upperContainer,function(){upperContainer.removeChild(menuBt);menu();});
					}
					...
					var mess= new createjs.Text("GOOD JOB!","bold 60px Geo","white");
					stage.removeChild(gameContainer);
					//upperContainer.addChild(mess);
					mess.regX=30;
					mess.x=315;
		...
		function gameOver()
		{
			...
			curtains.close(function(){
				...
				to({x:900},200,createjs.Ease.QuadIn).call(function(){
					...
					var mess= new createjs.Text("YOU SUCK!","bold 60px Geo","white");
					stage.removeChild(gameContainer);
					//upperContainer.addChild(mess);
					mess.regX=30;
					mess.x=315;
		...
		function UIGroup(){
			...
			this.update=function(){
				...
				beginFill(stack.PLAYER.levelBonds[stack.PLAYER.level].c).
		...
		function Curtains(){
			...
			tmp.graphics.setStrokeStyle(1).beginStroke("#000000").beginFill("#000000").
		...
		function eFactory()
		{
			...
			this.itemFrequency=
			{
				painKiller:{freq:20,maxFreq:170,add:true},
				candy0:    {freq:50,maxFreq:190,add:true},
				candy1:    {freq:50,maxFreq:230,add:true},
				candy2:    {freq:50,maxFreq:310,add:true},
				candy3:    {freq:50,maxFreq:330,add:true},
				candy4:    {freq:50,maxFreq:370,add:true},
				candy5:    {freq:50,maxFreq:390,add:true}
			};
				...
				for(var i in collisions)
					{
						...
						case "RugbyCop":
							...
							{
									var s=createjs.Sound.play("rugbyTouch");
						...
						case "NormalCop":
							...
							{
									var s=createjs.Sound.play("playerDamage");
							...
							switch(stack[collisions[i]].type)
								{
									...
									case "painKiller":
									var s=createjs.Sound.play("life");
									this.health+=20;
									points(stack.PLAYER.x,stack.PLAYER.y,"ph");
									...
									case "candy0":
									var s=createjs.Sound.play("candy");
									candyCash+=100;
					...
					if(this.level!=5){
						if(this.wackyness>=this.levelBonds[this.level].t){
							points("lu",stack.PLAYER.x,stack.PLAYER.y);
							...
							this.level++;
							var s=createjs.Sound.play("levelUp");
						}
  				...
  				if(this.health<=0){
  					var s=createjs.Sound.play("playerDeath");
  					gameOver();
  				}
  				...
  				if(p.bulletReady && p.fire)
					...
					p.bulletReady=false;
					var s=createjs.Sound.play("playerShoot");
					}
				...
				for(var i in collisions)
					{
						if(stack[collisions[i]].constructor.name=="Item" && stack[collisions[i]].type=="playerBullet")
							{
								var s=createjs.Sound.play("enemyDamage");
								this.health-=1;
								if(this.health<=0){
									var s=createjs.Sound.play("enemyDeath");
									this.time=0;
									this.state=2;
									}
								stack[collisions[i]].die();
		...
		function NormalCop(length,roadStart)
		{
			...
			this.shoot=function(vx,vy)
			{
				...
				var s=createjs.Sound.play("enemyShoot");
			}
			p.gotoAndPlay("run");
			entitiesContainer.addChild(p);
			...
			this.update=function()
			{
				...
				this.speed=-roadSpeed;
				if(p.currentAnimation!="die"){
				var s=createjs.Sound.play("enemyDie");
				p.gotoAndPlay("die");
				}
				if(this.time>10)
						this.die();				
		...
		function points (x,y,value) {
			...
			createjs.Tween.get(p).to({y:p.y-80},3000).call(function(){UIContainer.removeChild(p)});
